...
export const StoryListItem = (props: Props) => {
    ...
    function close(state) {
     ...
     slidePanel = () => {
        if(!open){
            ...
            _panel.hide()
            setOpen(!open)
            startAnimation(durationVideo-timeVideo) /*longueur vidéo moins le current time pour avoir le temps restant à lire*/
            setPlay(false)
        }
    }
    ...
    storyDelete = async () => {
        ...
        pushNewStory(story_id)
        props.onClosePress()
    }

    pushNewStory =  (id) =>{ 
        index = stories.findIndex(v => v.story_id === id) /* trouve l'index de la story a enlever */
        tab = stories

        if (index > -1) {
          tab.splice(index, 1); 
        }
         props.action([ { /* actualiser le state dans ProfileScreen */
    ...
    const videoStarted = (e) => {
        start(e)
        setDurationVideo(e)
    }

    const pressToPause = () => {
        startAnimation(durationVideo-timeVideo);
        setPlay(false)
    }
                    ...
                    <View style={styles.backgroundContainer}>
                        { content[current].image.includes("videoStory") ?(
                                <Video
                                    ...
                                    ref={video}
                                    style={styles.image}
                                    onLoad={ e => videoStarted(e.duration)}
                                    onProgress={play ? e => setTimeVideo (e.currentTime) : console.log('ok')}
                                    paused={play}
                                />
                        )
                        :
                ...
                <View style={{flexDirection: 'column', flex: 1,}}>
                    ...
                    <View style={styles.userContainer}>
                        <View style={{flexDirection: 'row', alignItems: 'center'}}>
                            ...
                            <TouchableOpacity
                                onPress={props.profileName === 'Sponsoring' ? () => console.log('Publicité') : getProfileInfos}
                            >
                        ...
                        <View style={[styles.bottom, {minHeight: screen.h/9 }]}>
                          ...
                          >
                            ...
                            {chat.length > 0 ? (
                                ...
                                >
                                    <Image
                                      source={require("./assets/images/bxs-sendWhite.png")}
                                      resizeMode={"contain"}
